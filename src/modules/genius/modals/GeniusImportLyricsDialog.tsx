	...
	const handleImport = useCallback(() => {
		...
		for (const lineText of lines) {
			...
			for (const part of parts) {
				...
				wordList.forEach((wordText, index) => {
					// 1. Add the actual word block
					words.push({
						id: uid(),
						word: wordText,
						startTime: 0,
					...
					// 2. If it's not the last word, insert a separate "Space x1" block right after it
					if (index < wordList.length - 1) {
						words.push({
							id: uid(),
							word: " ", // Text is empty for a space block
							startTime: 0,
							endTime: 0,
							emptyBeat: 1, // This triggers the faded [Space x1] UI!
						});
					}
				});
